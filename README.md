...
```sh
git clone https://github.com/abhazelton/dotfiles ~/.dotfiles && cd ~/.dotfiles && ./install.sh
```

Or, with nothing cloned yet — `install.sh` bootstraps itself:

```sh
sh -c "$(curl -fsSL https://raw.githubusercontent.com/abhazelton/dotfiles/main/install.sh)"
sh -c "$(curl -fsSL https://raw.githubusercontent.com/abhazelton/dotfiles/main/install.sh)" "" -y     # to pass flags, $0 needs a placeholder
```
...
| **tools** | one [mise](https://mise.jdx.dev) config installs node, bun, python, ruby, and `atuin bat delta eza fd fzf tmux zoxide`; `yarn` and `pnpm` come from corepack |
...
| `atuin.zsh` | the up key on atuin, and zsh's own history settings |
...
The prefix is <kbd>Ctrl</kbd>+<kbd>a</kbd>, not tmux's stock
<kbd>Ctrl</kbd>+<kbd>b</kbd> — <kbd>Ctrl</kbd>+<kbd>a</kbd>
<kbd>Ctrl</kbd>+<kbd>a</kbd> sends a literal Ctrl-a so beginning-of-line still
works. The cost is real: every tmux answer you find online says Ctrl-b and needs
translating. What else is not stock:
...
| <kbd>Alt</kbd>+arrows | move between panes with **no prefix at all** |
| <kbd>Alt</kbd>+<kbd>h</kbd> <kbd>l</kbd> | previous / next window, no prefix |
| <kbd>Alt</kbd>+<kbd>c</kbd> | new window in the current directory, no prefix |

The Alt bindings need the terminal to send Meta: on macOS that is iTerm2's
"Option as Meta" or Ghostty's `macos-option-as-alt`, or they arrive as nothing.
...
So there is **no `[user]` section here at all**, and no signing. Identity stays
per repository. Set a global one by hand if you want one, but note that
`~/.gitconfig` is a symlink into this repo, so `git config --global` writes
*through* it and turns up as a diff to commit. `~/.config/git/config` is the
untracked place for anything machine-specific — git reads it in addition to
`~/.gitconfig`, though *before* it, so the tracked file wins a clash.

## atuin: history that is present but invisible

Two settings, one of which is the single most confusing thing in this repo.

**`[search] shells = "all"`.** atuin records which shell ran each command, and
...
zsh's *own* history — a different file, empty on a machine that has only ever
run bash. Combined with the filter above, "up shows one entry and Ctrl+R shows
nothing" is the whole experience of arriving in zsh for the first time, and
neither half of it is history loss. In bash, atuin binds the up key itself; this
drop-in restores that.
...
correspond to something in `config/`, and the tracked tmux config lives at the
XDG path. Observed failure: a stale `unbind C-b` plus a tracked config that set
the prefix afterwards, leaving a session with no working prefix.
...
reason appending to them is allowed. It goes *above* step 25's `exec zsh -l`
trampoline so the line is reached before the handover.
